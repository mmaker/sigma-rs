...
mod tests {
    ...
    fn test_tag_affects_output() {
        let mut sponge1 = KeccakDuplexSponge::new(*b"domain-one-differs-here-00000000");
        let mut sponge2 = KeccakDuplexSponge::new(*b"domain-two-differs-here-00000000");
    ...
    fn test_multiple_blocks_absorb_squeeze() {
        let mut sponge = KeccakDuplexSponge::new(*b"multi-block-absorb-test_________");
        let input = vec![0xABu8; 3 * 200];

        sponge.absorb(&input);
        let output = sponge.squeeze(3 * 200);
